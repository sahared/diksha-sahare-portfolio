...
const TechStack = () => {
  ...
  return (
    <section
      id="tech"
      className="py-20 px-4 bg-[hsl(189,80%,68%)]/10 relative overflow-hidden border-t border-border/30"
    >
                ...
                <div className="flex flex-wrap gap-3">
                  {category.techs.map((tech, techIndex) => (
                    <Badge
                      key={techIndex}
                      variant="secondary"
                      className="px-4 py-2 text-sm text-teal-700 dark:text-teal-200 bg-teal-100 dark:bg-teal-800 hover:text-accent-foreground transition-all shadow-soft border border-border/50"
                    >
                      {tech}
                    </Badge>
                  ))}
