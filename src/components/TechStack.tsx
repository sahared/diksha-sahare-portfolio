...
const TechStack = () => {
  ...
  return <section id="tech" className="py-20 px-4 bg-[hsl(48,74%,78%)]/20 relative overflow-hidden border-t border-border/30">
      <div className="container mx-auto">
        <div className="max-w-4xl">
          <h2 className="text-4xl font-bold text-accent mb-4">Tech Stack</h2>
          <p className="text-muted-foreground mb-12">
            Change is inevitable, so I keep on exploring new technology, learn it in a minimal possible way and then build
            something out of it to see how well I did :)
          </p>
          ...
          <div className="space-y-8">
            {categories.map((category, index) => <div key={index}>
                ...
                <div className="flex flex-wrap gap-3">
                  {category.techs.map((tech, techIndex) => <Badge key={techIndex} variant="secondary" className="px-4 py-2 text-sm hover:text-accent-foreground transition-all shadow-soft border border-border/50 bg-inherit">
                      {tech}
                    </Badge>)}
                </div>
              </div>)}
