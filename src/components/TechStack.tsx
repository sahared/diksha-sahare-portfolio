...
const TechStack = () => {
  ...
  return (
    <section
      id="tech"
      className="py-20 px-4 bg-[hsl(210,100%,50%)]/10 relative overflow-hidden border-t border-border/30"
    >
                ...
                <div className="flex flex-wrap gap-3">
                  {category.techs.map((tech, techIndex) => (
                    <Badge
                      key={techIndex}
                      variant="secondary"
                      className="px-4 py-2 text-sm hover:text-accent-foreground transition-all shadow-soft border border-border/50 bg-inherit"
                    >
                      {tech}
                    </Badge>
                  ))}
